// API utility functions for interacting with various endpoints in the application
 ...
 * @param {number} steps - The number of steps to filter by (optional).
 ...
 * @param {string} ingredientsMatchType - Type of ingredients matching (e.g., "any" or "all").
 * @param {string} sortBy - The field to sort by (e.g., "rating", "name").
 * @param {string} sortOrder - The order of sorting (e.g., "asc" for ascending, "desc" for descending).
 * @returns {Promise<Object>} - A promise resolving to an array of recipes or an error message.
 */
export async function fetchRecipes(
  ...
  try {
    ...
    if (topRated) url.searchParams.append("topRated", "true");
    if (ingredients.length > 0) url.searchParams.append("ingredients", ingredients.join(","));
    if (ingredientsMatchType) url.searchParams.append("ingredientsMatchType", ingredientsMatchType);
...
/**
 * Fetches a single product by its ID from the '/api/recipes/[id]' endpoint.
 *
 * @async
 * @function fetchProductById
 * @param {string} id - The ID of the product to fetch.
 * @returns {Promise<Object>} - A promise resolving to the product data or an error message.
 */
export async function fetchProductById(id) {
  try {
    const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/recipes/${id}`;
    const response = await fetch(url);
 ...
 * @returns {Promise<Object>} - A promise resolving to an array of categories or an error message.
 */
...
/**
 * Fetches tags from the '/api/tags' endpoint.
 *
 * @async
 * @function fetchTags
 * @returns {Promise<Object>} - A promise resolving to an array of tags or an error message.
 */
export async function fetchTags() {
  try {
    const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/tags`;
    const response = await fetch(url);
 ...
 * @returns {Promise<Object>} - A promise resolving to the reviews data or an error message.
 */
export async function fetchReviews(recipeId) {
  try {
    ...
    return { error: `Error fetching reviews: ${error.message}` };
  }
}
 ...
 * @returns {Promise<Object>} - A promise resolving to an array of ingredients or an error message.
 */
export async function fetchIngredients() {
  try {
    const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/ingredients`;
    const response = await fetch(url);
