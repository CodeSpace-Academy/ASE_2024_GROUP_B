 ...
 * @param {string} sort - Sorting criteria (optional).
 ...
 */
export async function fetchRecipes(page, limit, search, category, tags, steps, ingredients = [], topRated = false, sort,ingredientsMatchType) {
  try {
    // Construct the URL
    ...
    if (ingredientsMatchType) {
      ...
      if (sort) 
        {url.searchParams.append("sort", sort); // Append sorting criteria
...
/**
 * Fetches sorted categories or other datasets (example implementation for clarity).
 * 
 * This function sends a GET request with sorting and filtering capabilities.
 * Extend it for specific use cases where sorted data retrieval is needed.
 * 
 * @async
 * @function fetchSortedData
 * @param {string} endpoint - The endpoint to fetch data from.
 * @param {string} sort - Sorting criteria (optional).
 * @returns {Promise<Object>} - A promise that resolves to the sorted data or an error message if failed.
 */
export async function fetchSortedData(endpoint, sort) {
  try {
    const url = `${process.env.NEXT_PUBLIC_BASE_URL || process.env.BASE_URL}/${endpoint}`;
    const queryParams = new URLSearchParams();
    if (sort) queryParams.append("sort", sort);

    console.log("Fetching sorted data from:", url + "?" + queryParams.toString());
    const response = await fetch(`${url}?${queryParams.toString()}`);

    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.message || "Failed to fetch sorted data");
    }
    ...
    return { error: `Error fetching sorted data: ${error.message}` };
  }
}